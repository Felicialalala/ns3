...
{
    //NS_LOG_UNCOND("调用RxPacketTraceUeCallback");
    phyStats->m_lastTblerDl = params.m_tbler; // 存储下行tbler
    if (!m_rxPacketTraceFile.is_open())
    {
        std::ostringstream oss;
...
NrPhyRxTrace::RxPacketTraceGnbCallback(Ptr<NrPhyRxTrace> phyStats,
                                       ...
                                       RxPacketTraceParams params)
{
    phyStats->m_lastTblerUl = params.m_tbler; // 存储上行tbler
    //NS_LOG_UNCOND("NrPhyRxTrace::RxPacketTraceGnbCallback...");
    if (!m_rxPacketTraceFile.is_open())
    {
        std::ostringstream oss;
        oss << m_resultsFolder << "RxPacketTrace" << m_simTag.c_str() << ".txt";
...
double 
NrPhyRxTrace::GetLastTbler() const
{
  return m_lastTblerDl; // 根据需求返回下行或上行值
}
