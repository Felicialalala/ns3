...
// 在nr-helper.cc中实现 new
double
NrHelper::GetLastUeTbler() const
{
  return m_phyStats->GetLastTbler();
}
    ...
    const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>> allBwps)
{
    ...
    for (uint32_t bwpId = 0; bwpId < allBwps.size(); ++bwpId)
    {
        ...
        cc->SetUlBandwidth(static_cast<uint16_t>(bwInKhz / 100));
        cc->SetDlBandwidth(static_cast<uint16_t>(bwInKhz / 100));
        NS_LOG_UNCOND("UlBandwidth:"<<static_cast<uint16_t>(bwInKhz / 100));
...
// int64_t NrHelper::DoAssignStreamsToChannelObjects(Ptr<NrSpectrumPhy> phy, int64_t currentStream)
// {
//     int64_t initialStream = currentStream;

//     // 获取 SpectrumChannel
//     Ptr<SpectrumChannel> channel = phy->GetSpectrumChannel();
//     NS_LOG_UNCOND("SpectrumChannel pointer: " << (channel ? "Valid" : "Null"));
//     NS_ASSERT(channel != nullptr);

//     // 获取 PropagationLossModel
//     Ptr<PropagationLossModel> lossModel = channel->GetPropagationLossModel();
//     NS_LOG_UNCOND("PropagationLossModel pointer: " << (lossModel ? "Valid" : "Null"));
//     NS_ASSERT(lossModel != nullptr);

//     // 尝试将 PropagationLossModel 转换为 ThreeGppPropagationLossModel
//     Ptr<ThreeGppPropagationLossModel> propagationLossModel = DynamicCast<ThreeGppPropagationLossModel>(lossModel);
//     if (propagationLossModel)
//     {
//         NS_LOG_UNCOND("PropagationLossModel is ThreeGppPropagationLossModel.");
...
//     else
//     {
//         NS_LOG_UNCOND("PropagationLossModel is not ThreeGppPropagationLossModel, using base model.");
//         currentStream += lossModel->AssignStreams(currentStream);
//     }

//     // 获取 ChannelConditionModel
//     Ptr<ChannelConditionModel> channelConditionModel = nullptr;
//     if (propagationLossModel)
//     {
//         channelConditionModel = propagationLossModel->GetChannelConditionModel();
//     }
//     NS_LOG_UNCOND("ChannelConditionModel pointer: " << (channelConditionModel ? "Valid" : "Null"));
//     if (channelConditionModel && std::find(m_channelObjectsWithAssignedStreams.begin(),
//                                            m_channelObjectsWithAssignedStreams.end(),
...
//     // 获取 PhasedArraySpectrumPropagationLossModel
//     Ptr<ThreeGppSpectrumPropagationLossModel> spectrumLossModel =
//         DynamicCast<ThreeGppSpectrumPropagationLossModel>(channel->GetPhasedArraySpectrumPropagationLossModel());
//     NS_LOG_UNCOND("PhasedArraySpectrumPropagationLossModel pointer: " << (spectrumLossModel ? "Valid" : "Null"));
//     if (spectrumLossModel)
//     {
//         Ptr<ThreeGppChannelModel> channelModel = DynamicCast<ThreeGppChannelModel>(spectrumLossModel->GetChannelModel());
//         NS_LOG_UNCOND("ThreeGppChannelModel pointer: " << (channelModel ? "Valid" : "Null"));
//         if (channelModel && std::find(m_channelObjectsWithAssignedStreams.begin(),
//                                       m_channelObjectsWithAssignedStreams.end(),
//                                       channelModel) == m_channelObjectsWithAssignedStreams.end())
//         {
//             currentStream += channelModel->AssignStreams(currentStream);
//             m_channelObjectsWithAssignedStreams.emplace_back(channelModel);
//         }
//     }
...
{
    NS_LOG_FUNCTION(this);
    //NS_LOG_UNCOND("EnableDlDataPhyTraces");
...
{
    NS_LOG_FUNCTION(this);
    //NS_LOG_UNCOND("EnableUlPhyTraces");
    Config::Connect(
