...
void ConfigurePositions(NodeContainer& gNbNodes, NodeContainer& ueNodes, MobilityHelper& mobility, uint16_t gNbNum, uint16_t ueNum)
{
    ...
    double ueHeight = 1.5;
    double cellRadius = 500; // 单基站覆盖半径或蜂窝边长
    ...
    else 
    {
        ...
        for (uint32_t i = 0; i < ueNodes.GetN(); ++i)
        {
            ...
            std::uniform_real_distribution<double> radiusDist(0, cellRadius);
...
main(int argc, char* argv[])
{
    ...
    uint32_t lambdaUll = 10;
    uint32_t lambdaBe = 10;
    ...
    ConfigurePositions(gNbNodes, ueNodes, mobility, gNbNum, ueNumPergNb * gNbNum);
