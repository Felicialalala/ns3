...
#include "ns3/antenna-module.h"//配置天线模型，包括MIMO相关设置
#include "ns3/applications-module.h"//应用层流量生成器
...
//绑定 SINR 和 MAC 调度器的 TraceSink
void DlSinrTraceFunction(std::string context, double sinrValue) {
    NS_LOG_UNCOND("DL SINR: " << sinrValue << " Context: " << context);
}

void DlMacSchedulerTraceFunction(std::string context, uint32_t ueId, uint32_t rbAllocated) {
    NS_LOG_UNCOND("UE: " << ueId << " RB Allocated: " << rbAllocated << " Context: " << context);
}

void PrintDeviceAttributes(Ptr<NetDevice> device, std::string basePath) {
    Config::MatchContainer subPaths = Config::LookupMatches(basePath + "/*");
    for (uint32_t i = 0; i < subPaths.GetN(); ++i) {
        std::cout << "Sub-path: " << subPaths.GetMatchedPath(i) << std::endl;
    }
}
...
main(int argc, char* argv[])
{
    ...
    uint16_t gNbNum = 1;//initial value,  can be modified by args
    ...
    // ns-3 Time values which use integers to av   oid portability issues.
    ...
    double bandwidthBand1 = 50e6;//50MHz
    ...
    std::string outputDir = "./output/mimo";
    ...
    /*
     * Attributes of ThreeGppChannelModel still cannot be set in ourF way.
    ...
    // Config::Connect("/NodeList/0/DeviceList/0/NrPhy/DlSinr", MakeCallback(&DlSinrTraceFunction));
    // Config::Connect("/NodeList/1/DeviceList/0/NrPhy/DlSinr", MakeCallback(&DlSinrTraceFunction));
    // Config::Connect("/NodeList/0/DeviceList/0/NrMac/DlScheduling", MakeCallback(&DlMacSchedulerTraceFunction));
    
    // 遍历可能的路径
    Config::MatchContainer allPaths = Config::LookupMatches("/NodeList/*/DeviceList/*");
    for (uint32_t i = 0; i < allPaths.GetN(); ++i) {
        std::cout << "Available Path: " << allPaths.GetMatchedPath(i)<< std::endl;
    } 
    // 遍历每个节点的设备
    for (uint32_t nodeId = 0; nodeId < NodeList::GetNNodes(); ++nodeId) {
        ...
        for (uint32_t devId = 0; devId < node->GetNDevices(); ++devId) {
            ...
            std::cout << "  Device ID: " << devId
                      ...
                      << " Path: /NodeList/" << nodeId << "/DeviceList/" << devId << std::endl;
        }
    }
    //确认 NrPhy 是否存在于设备的路径中。以下代码将遍历设备，尝试打印所有子路径。
     for (uint32_t nodeId = 0; nodeId < NodeList::GetNNodes(); ++nodeId) {
        Ptr<Node> node = NodeList::GetNode(nodeId);
        for (uint32_t devId = 0; devId < node->GetNDevices(); ++devId) {
            Ptr<NetDevice> device = node->GetDevice(devId);
            if (device->GetInstanceTypeId() == TypeId::LookupByName("ns3::NrGnbNetDevice") ||
                device->GetInstanceTypeId() == TypeId::LookupByName("ns3::NrUeNetDevice")) {
                std::string basePath = "/NodeList/" + std::to_string(nodeId) + "/DeviceList/" + std::to_string(devId);
                std::cout << "Inspecting device path: " << basePath << std::endl;
                PrintDeviceAttributes(device, basePath);
            }
        }
    }
    // // 获取 gNB 的 PHY
    // Ptr<NrPhy> gnbPhy = nrHelper->GetGnbPhy(gnbNetDev.Get(0), 0);
    // gnbPhy->TraceConnectWithoutContext("DlSinr", MakeCallback(&DlSinrTraceFunction));

    // // 获取 UE 的 PHY
    // Ptr<NrPhy> uePhy = nrHelper->GetUePhy(ueLowLatNetDev.Get(0), 0);
    // uePhy->TraceConnectWithoutContext("DlSinr", MakeCallback(&DlSinrTraceFunction));

    // //验证 TraceSink 是否绑定成功
    // bool connected = gnbPhy->TraceConnectWithoutContext("DlSinr", MakeCallback(&DlSinrTraceFunction));
    // if (connected) {
    //     std::cout << "TraceSink successfully connected to DlSinr." << std::endl;
    // } else {
    //     std::cout << "Failed to connect TraceSink to DlSinr." << std::endl;
    // }
...
//             std::string path = "/NodeList/" + std::to_string(nodeId) + "/DeviceList/" +
//                                std::to_string(devId) + "/NrPhy/DlSinr";
//             Config::Connect(path, MakeCallback(&DlSinrTraceFunction));
//             std::cout << "Connected TraceSink to: " << path << std::endl;
//         }
//     }
// }
    ...
    // // 绑定 TraceSink 到 TraceSource
    // Config::Connect("/NodeList/*/DeviceList/*/NrPhy/DlSinr",
    //                 MakeCallback(&DlSinrTraceFunction));
    // Config::Connect("/NodeList/*/DeviceList/*/NrMac/DlScheduling",
    //                 MakeCallback(&DlMacSchedulerTraceFunction));
