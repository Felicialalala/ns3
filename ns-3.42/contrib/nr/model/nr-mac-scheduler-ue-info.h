...
class NrMacSchedulerUeInfo
{
  public:
     ...
     */
    NrMacSchedulerUeInfo(uint16_t rnti, BeamId beamId, uint16_t cellId, const GetRbPerRbgFn& fn);
    ...
    struct CqiInfo
    {
        ...
        double m_tbler;  // Transport Block Error Rate,new                  
    };
    ...
    uint8_t m_startMcsDlUe{0};    //!< Starting DL MCS to be used
     ...
     // 新增
    double m_lastDlTbler{0.0}; // 默认初始化为 0.0
    double m_lastUlTbler{0.0}; // 默认初始化为 0.0
    uint16_t m_cellId{0}; //!< Cell ID to which the UE is attached
