...
const std::unordered_map<uint16_t, std::shared_ptr<NrMacSchedulerUeInfo>>&
NrMacSchedulerNs3::GetUeMap() const
{
    return m_ueMap;
}
    ...
    const NrMacCschedSapProvider::CschedCellConfigReqParameters& params)
{
    ...
    //NS_LOG_UNCOND("m_bandwidth:"<<m_bandwidth);
    ...
    const NrMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
    ...
    if (itUe == m_ueMap.end())
    {
        ...
        UeInfoOf(*itUe)->m_cellId = GetCellId();
    ...
    const NrMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
    ...
    for (const auto& cqi : params.m_cqiList)
    {
        ...
        if (cqi.m_cqiType == DlCqiInfo::WB)
        {
            ...
            //m_cqiManagement.DlSBCQIReported(cqi, ue, expirationTime, m_maxDlMcs, m_macSchedSapUser->GetSpectrumModel());
        }
        else
        {
            // m_cqiManagement.DlSBCQIReported(cqi, ue);
            m_cqiManagement.DlWBCQIReported(cqi, ue, expirationTime, m_maxDlMcs);//反转
        }
    }
}
    ...
    const NrMacSchedSapProvider::SchedUlCqiInfoReqParameters& params)
{
    ...
    // 打印日志，查看当前rbg大小设置值
    //NS_LOG_INFO("Using RBG size of " << m_macSchedSapUser->GetSpectrumModel() << " RBs");
    ...
    const std::string& direction) const
{
         ...
         /* nothing as increment */)
    {
        ...
        // 修改 只允许重传一次
        //uint8_t maxHarqReTx = m_enableHarqReTx ? 1 : 0;
...
{
    std::vector<uint8_t> rbgBitmask(GetBandwidthInRbg(), 1);
    //NS_LOG_UNCOND("rbgBitmask.size():"<<rbgBitmask.size());
 ...
 * \param spoint Starting point of the blocks to add to the allocation list(表示当前调度的时频资源起始位置（子载波、符号）。m_rbg, m_sym,分别表示在频率/时间维度上的资源块组索引，用于确定在频率、时间域上的起始位置。)
 * \param symAvail Number of available symbols, 当前时隙可用的符号数量。
 * \param activeDl List of active UE with data to transmit in DL, 活跃 UE 的映射表（波束 → UE 列表）
 * \param slotAlloc The allocation info to which append the allocations,存储调度结果的时隙分配信息
 ...
 * 该函数是 MAC 调度器的核心功能之一，负责下行数据资源分配。
 * 其主要流程为：根据可用符号、活跃 UE 的资源需求，为每个 UE 分配物理资源（符号、RBG），生成下行控制信息（DCI），并将调度结果传递给物理层。
 */
uint8_t
                                    ...
                                    SlotAllocInfo* slotAlloc) const
{
    ...
    NS_ASSERT(spoint->m_rbg == 0);//确保调度从资源块组（RBG）0 开始
    // 通过 AssignDLRBG 按波束分配符号，确保公平性。
    //- 每个 UE 的 DCI 生成需满足 TB 尺寸 ≥ 7 字节（3 字节 MAC 头 + 2 字节 RLC 头 + 2 字节数据）。
    BeamSymbolMap symPerBeam = AssignDLRBG(symAvail, activeDl);//将可用符号分配给不同波束，返回每个波束的符号数量。 
    GetFirst GetBeam;
    uint8_t usedSym = 0;
    ...
    for (const auto& beam : activeDl)
    {
        ...
        for (const auto& ue : beam.second)
        {
            ...
            // 为 UE 生成 DCI，包含资源分配、调制编码策略（MCS）等信息。
            ...
            //NS_LOG_UNCOND( "Step 1 (DCI生成) took: " << elapsed1.count() << " ms\n");            
            // 3. 分配 HARQ 进程
            HarqProcess harqProcess(true, HarqProcess::WAITING_FEEDBACK, 0, dci);// 创建新的 HARQ 进程，状态为等待反馈
            uint8_t id;
            ...
            //NS_LOG_UNCOND( "Step 3 (逻辑信道资源分配) took: " << elapsed3.count() << " ms\n");
        ...
        if (assigned)
        {
            ...
            // 6. 更新符号使用量
            usedSym += allocSym;// 该波束下所有 UE 分配的符号总数。
 ...
 * 负责上行数据资源分配。
 * 其主要流程为：根据可用符号、活跃 UE 的资源需求，为每个 UE 分配物理资源（符号、RBG），生成上行控制信息（DCI），并将调度结果传递给物理层。
 */
uint8_t
                                    ...
                                    SlotAllocInfo* slotAlloc) const
{
    ...
    for (const auto& beam : activeUl)
    {
        ...
        for (const auto& ue : beam.second)
        {
            ...
            auto end1 = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> elapsed1 = end1 - start1;
            //( "Step 1 (DCI生成) took: " << elapsed1.count() << " ms\n");
            // 分配 HARQ 进程
            ...
            auto end3 = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> elapsed3 = end3 - end2;
            //( "Step 3 (逻辑信道资源分配) took: " << elapsed3.count() << " ms\n");
 ...
 * MAC 调度器的上行调度核心
 * 负责分配上行资源（符号、RBG）给 HARQ 重传、随机接入消息（Msg3）、调度请求（SR）和新数据。
 * 其遵循 TDMA 机制，资源分配从最后一个符号倒序进行，确保时隙结构为：DL 控制 → DL 数据 → UL 数据 → UL 控制。
 */
uint8_t
NrMacSchedulerNs3::DoScheduleUl(const std::vector<UlHarqInfo>& ulHarqFeedback,//上行 HARQ 反馈列表（ACK/NACK）
                                const SfnSf& ulSfn,//当前调度的上行时隙
                                SlotAllocInfo* allocInfo,//存储调度结果的时隙分配信息
                                LteNrTddSlotType type)//时隙类型（如 F 类型、UL 类型）
{
    NS_LOG_FUNCTION(this);
    ...
    auto end1 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> elapsed1 = end1 - start1;
    //std::cout << "随机接入前took: " << elapsed1.count() << " ms\n";
    // 5. 调度随机接入 Msg3
    uint8_t usedMsg3 = 0;
    if (!m_rachList.empty())
    {
        // 为随机接入的 Msg3 分配符号，确保竞争解决
    ...
    auto end3 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> elapsed3 = end3 - end2;
    //std::cout << "重传+SR调度 took: " << elapsed3.count() << " ms\n";

    // 8. 调度新数据
    ActiveUeMap activeUlUe;
    // 筛选有上行数据且未被 HARQ/SR 占用的 UE
    ...
    //std::cout << "数据调度took: " << elapsed4.count() << " ms\n";
