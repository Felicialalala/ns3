...
{
    // Calculate the interference+noise (I+N) covariance matrix for this signal,计算干扰加噪声协方差矩阵
    // including interference from other RX signals 考虑了小区外干扰和其他可能的干扰信号
    ...
    // 函数对信道矩阵进行干扰白化处理，使得干扰加噪声协方差矩阵变为单位矩阵，便于后续计算。
    ...
    // 如果 rxSignal 包含预编码矩阵，则直接使用；否则，创建一个简单的预编码矩阵。
    ...
    // 函数根据预编码矩阵和经过干扰白化处理的信道矩阵计算 MIMO 系统中的 SINR 矩阵。
