 ...
 * NrChunkProcessor 类在 NS-3 网络仿真框架中用于处理接收到的 NR（New Radio，新无线电）信号的时频 SINR（信号与干扰加噪声比）、干扰或功率块。
 */
class NrChunkProcessor : public SimpleRefCount<NrChunkProcessor>
{
  public:
    NrChunkProcessor();//构造函数，用于初始化 NrChunkProcessor 对象。
    virtual ~NrChunkProcessor(); // 虚析构函数，确保在删除派生类对象时能正确释放资源。
  ...
  private:
    Ptr<SpectrumValue> m_sumValues; ///< sum values 用于累加每次评估信号块时传入的频谱值（如 SINR）
    Time m_totDuration;             ///< total duration 记录所有评估信号块的总持续时间

    // 存储用户添加的回调函数列表，这些回调函数将在处理结束时被调用
