...
#include <sstream>  // 字符串流操作（已在使用 ostringstream）
...
NrSpectrumPhy::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrSpectrumPhy")
            ...
            .AddTraceSource("BlerTrace",
                "Reports the BLER for each received transport block",
                MakeTraceSourceAccessor(&NrSpectrumPhy::m_blerTrace),
                "ns3::BlerTraceCallback")
            ...
            .AddTraceSource("RxPacketTraceGnb",
                            "The no. of packets received and transmitted by the Base Station",
                            MakeTraceSourceAccessor(&NrSpectrumPhy::m_rxPacketTraceGnb),
                            "ns3::RxPacketTraceParamsWithRbBitmap::TracedCallback")
...
// set callbacks
// 以下是各种设置回调的函数，这些函数用于设置在不同事件发生时调用的回调，例如接收数据、接收控制信息、接收 PSS、接收 HARQ 反馈等时的回调
void
...
// 继承自 SpectrumPhy 的一些设置和获取函数，如设置设备、获取设备、设置移动性、获取移动性、设置信道、获取接收频谱模型等
void
...
NrSpectrumPhy::StartRx(Ptr<SpectrumSignalParameters> params)
{
    ...
    if (nrDataRxParams)// 数据帧处理
    {
        if (nrDataRxParams->cellId == GetCellId())
        {
            ...
            // [3.1] 强制接收所有数据帧（用于CQI反馈）
            // 这段代码的目的是为了确保所有的UE都能从数据信号中获取到CQI反馈，即使这些信号是为其他UE发送的。
            isIntendedRx = true;

            if (isIntendedRx)
            {
                NS_LOG_INFO("start Rx Data!!!");
                StartRxData(nrDataRxParams);//重点
            }
            // [3.2] 下行路径损耗追踪（仅UE侧）
    ...
    else if (dlCtrlRxParams != nullptr)// 下行控制处理
    {
    ...
    else if (ulCtrlRxParams != nullptr)// 上行控制处理
    {
    ...
    {
        MaybeCcaBusy();// 检测信道是否繁忙
    }
}

// 新增，用于打印全rb上的信道矩阵
void
NrSpectrumPhy::PrintSpectrumChannelMatrix(){
    NS_LOG_FUNCTION(this);
}
                                      ...
                                      const Time& duration)
{
    ...
    case IDLE: {
        ...
        if (m_channel)
        {
            m_channel->StartTx(txParams);//需要进一步查看
        }
        else
        {
            NS_LOG_WARN("Working without channel (i.e., under test)");
...
//UpdateSrsSinrPerceived函数接收SpectrumValue类型的srsSinr参数，用于更新基站接收到的 SRS 信号的SINR
void
...
NrSpectrumPhy::StartRxData(const Ptr<NrSpectrumSignalParametersDataFrame>& params)
{
    ...
    bool printAllRb = false;
    // 新增信道矩阵调试输出
    if (params->allRbSpectrumChannelMatrix && printAllRb)
    {
        NS_LOG_DEBUG("==== Channel Matrix for Cell " << params->cellId 
                    << " User RNTI " << params->rnti << " =====");
        const size_t numRxPorts = params->allRbSpectrumChannelMatrix->GetNumRows();   // RX天线数
        const size_t numTxPorts = params->allRbSpectrumChannelMatrix->GetNumCols();   // TX天线数
        const size_t numRbs = params->allRbSpectrumChannelMatrix->GetNumPages();      // RB数量

        NS_LOG_DEBUG("Channel Matrix Dimensions: RxPorts=" << numRxPorts 
            << ", TxPorts=" << numTxPorts 
            << ", RBs=" << numRbs);
        
        // 验证矩阵维度合法性
        NS_ABORT_IF(numRxPorts * numTxPorts * numRbs != params->allRbSpectrumChannelMatrix->GetSize());

        // 创建日志文件句柄（使用静态变量减少重复打开）
        static std::ofstream outfile;
        const std::string filename = "/home/yangyiting/workspace/ns-3-allinone/ns-3.42/output/channel_matrix.csv";

        // 首次打开时初始化文件
        if (!outfile.is_open()) {
            outfile.open(filename, std::ios::out | std::ios::app); // 改为追加模式打开
            outfile << "cell_id,rnti,rb,rx,tx,real,imag\n"; // 文件头只写一次
        }

        const double epsilon = 1e-15; // 零值阈值

        for (size_t rb = 0; rb < numRbs; ++rb) {
            for (size_t rx = 0; rx < numRxPorts; ++rx) {
                for (size_t tx = 0; tx < numTxPorts; ++tx) {
                    const auto& h = params->allRbSpectrumChannelMatrix->operator()(rx, tx, rb);
                    const double magnitude = std::abs(h);
                    
                    if (magnitude < epsilon) continue;
                    
                    outfile << params->cellId << ","
                            << params->rnti << ","
                            << rb << ","
                            << rx << ","
                            << tx << ","
...
//新增，加入带per-RB SINR的输出（结合m_rbBitmap，已知这次TB实际使用哪些RB）
void
TransportBlockInfo::UpdatePerceivedSinr(const SpectrumValue& perceivedSinr)
{
    m_sinrAvg = 0.0;
    ...
    // 新增SINR输出
    // std::cout << "========= RNTI " << m_expected.m_rnti << " SINR per RB =========" << std::endl;

    // for (const auto& rbIndex : m_expected.m_rbBitmap)
    // {
    //     double sinrLinear = perceivedSinr.ValuesAt(rbIndex);
    //     double sinrDb = 10 * std::log10(sinrLinear + 1e-12); // 避免log(0)

    //     m_sinrAvg += sinrLinear;
    //     if (sinrLinear < m_sinrMin)
    //     {
    //         m_sinrMin = sinrLinear;
    //     }

    //     std::cout << "  RB[" << rbIndex << "] = " << sinrDb << " dB (linear: " << sinrLinear << ")" << std::endl;
    // }

    // m_sinrAvg = m_sinrAvg / m_expected.m_rbBitmap.size();

    // std::cout << "→ SINR Avg (linear): " << m_sinrAvg
    //           << ", SINR Min (linear): " << m_sinrMin
    //           << ", SINR Avg (dB): " << 10 * log10(m_sinrAvg + 1e-12)
    //           << std::endl;
    // std::cout << "==============================================================" << std::endl;

    // NS_LOG_INFO("Finishing RX, sinrAvg=" << m_sinrAvg
    //                                      << " sinrMin=" << m_sinrMin
    //                                      << " SinrAvg (dB) " << 10 * log10(m_sinrAvg + 1e-12));
}
...
NrSpectrumPhy::CheckTransportBlockCorruptionStatus()
{
    for (auto& tbIt : m_transportBlocks)
    {
        ...
        // if(tbInfo.m_outputOfEM->m_tbler>0.1){
        //     tbInfo.m_isCorrupted = true;
        // }
        


        // 触发 BLER 追踪
        m_blerTrace(GetCellId(), rnti, tbInfo.m_expected.m_harqProcessId, tbInfo.m_outputOfEM->m_tbler);
        ...
        for (auto packet : packetBurst->GetPackets())
        {
            ...
            else if (ueRx)
            {
                ...
                RxPacketTraceParams traceParams(tbInfo,
                                                ...
                                                cqi);
                                                // 验证 rbBitmap 是否被正确填充
                NS_LOG_UNCOND("TB RNTI " << rnti << " uses RBs: " << traceParams.m_rbBitmap.size());
