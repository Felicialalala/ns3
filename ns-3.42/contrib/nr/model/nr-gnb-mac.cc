...
{
    // 防止对 m_numRbPerRbg 属性进行多次配置，确保该属性只能被设置一次，以避免在运行时意外修改该属性
...
// const SfnSf& sfnSf：表示系统帧号和子帧号（System Frame Number and Sub - Frame Number）的对象，用于标识当前的时隙。
// LteNrTddSlotType type：表示时隙类型的枚举值，用于指示当前时隙的类型（如下行、上行或特殊时隙）。
void
NrGnbMac::DoSlotDlIndication(const SfnSf& sfnSf, LteNrTddSlotType type)
{
    ...
    // Send Dl-CQI info to the scheduler    if(m_dlCqiReceived.size () > 0)
    {
        ...
        // 将 m_dlCqiReceived 中的所有 CQI 信息复制到 dlCqiInfoReq.m_cqiList 中，并清空 m_dlCqiReceived 容器。
        ...
        for (const auto& v : dlCqiInfoReq.m_cqiList)
        {
            ...
            msg->SetDlCqi(v);
            //NS_LOG_UNCOND(" dlCqiInfoReq.m_cqiList.m_sinr: "<<v.m_sinr);
...
//是一个关键的上行链路（UL）时隙指示函数。它通常由底层物理层（PHY）在每次上行时隙开始时调用，以通知 MAC 层当前时隙的开始。
//这个函数的主要职责是处理与上行链路相关的调度和资源分配。
//sfnSf:系统帧号和子帧号（System Frame Number and Sub - Frame Number），用于标识当前的时隙。
void
NrGnbMac::DoSlotUlIndication(const SfnSf& sfnSf, LteNrTddSlotType type)
{
    ...
    // 2. 发送上行信道质量指示（UL - CQI）信息
    for (auto& i : m_ulCqiReceived)
    {
        // m_ulCqiReceived.at (i).m_sfnSf = ((0x3FF & frameNum) << 16) | ((0xFF & subframeNum) << 8)
        // | (0xFF & varTtiNum);
        // 对于每个接收到的 UL - CQI 信息 i，调用 m_macSchedSapProvider 的 SchedUlCqiInfoReq 函数将其发送给调度器。
    ...
    // 3. 发送调度请求（SR）信息
    {
        ...
        // 将 m_srRntiList 中的所有调度请求信息插入到 params.m_srList 中，然后清空 m_srRntiList。
        params.m_srList.insert(params.m_srList.begin(), m_srRntiList.begin(), m_srRntiList.end());
        m_srRntiList.clear();

        // 调用 m_macSchedSapProvider 的 SchedUlSrInfoReq 函数将调度请求信息发送给调度器。
        m_macSchedSapProvider->SchedUlSrInfoReq(params);

        // 遍历 params.m_srList，为每个调度请求创建一个 NrSRMessage 对象，并设置其 RNTI
    ...
    if (!m_ulCeReceived.empty())
    {
        ...
        // 遍历 ulMacReq.m_macCeList，为每个上行缓冲状态报告创建一个 NrBsrMessage 对象，并设置其 BSR（Buffer Status Report）。
...
NrGnbMac::DoDlHarqFeedback(const DlHarqInfo& params)
{
    ...
    if (params.m_harqStatus == DlHarqInfo::ACK)
    {
        ...
        NS_LOG_INFO(this << " HARQ-ACK UE RNTI" << params.m_rnti << " HARQ Process ID "
...
NrGnbMac::DoTransmitPdu(NrMacSapProvider::TransmitPduParameters params)
{
    ...
    // 通过对 params.rnti 和 params.harqProcessId 进行位运算，生成一个唯一的 TB 映射键 tbMapKey。这个键用于在 m_macPduMap 中查找对应的 MAC PDU 存储元素。
...
NrGnbMac::DoConfigureMac(uint16_t ulBandwidth, uint16_t dlBandwidth)
{
    ...
    NS_LOG_UNCOND("m_bandwidthInRbg: " << m_bandwidthInRbg << " RBG");

    NS_LOG_UNCOND("Mac configured. Attributes:"
                 << std::endl
