...
class NrSpectrumPhy : public SpectrumPhy
{
  public:
    /**
     * 获取对象的 TypeId，这是 ns3 中用于对象类型识别和属性管理的机制
    ...
    /**
     * \brief NrSpectrumPhy constructor 构造函数
     */
    NrSpectrumPhy();

    /**
     * \brief ~NrSpectrumPhy 析构函数
     */
    ...
    enum State
    {
        IDLE = 0,   //!< 空闲状态，没有正在进行的操作
        TX,         //!< 传输状态（数据或控制信息） (data or ctrl)
        RX_DATA,    //!< 接收数据
        RX_DL_CTRL, //!< 接收下行控制信息 DL CTRL
        RX_UL_CTRL, //!< 接收上行控制信息 UL CTRL
        RX_UL_SRS,  //!< 接收 SRS 信号
        CCA_BUSY    //!< 信道被其他实体占用的繁忙状态
    };

    // callbacks typefefs and setters 回调类型定义及设置函数
    /**
    ...
    // Methods inherited from spectrum phy 继承自 SpectrumPhy 的方法
    ...
    // 新增，用于打印全rb上的信道矩阵
    void PrintSpectrumChannelMatrix();
    /**
  ...
  private:
    ...
    // 用于BLER追踪计算
    TracedCallback<uint64_t, uint64_t, uint8_t, double> m_blerTrace;
