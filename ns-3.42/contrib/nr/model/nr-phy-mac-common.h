...
struct DlCqiInfo
{
    //std::vector<double> m_sinr;// new
    SpectrumValue m_sinr; // 新增成员变量用于存储 SINR
...
struct TransportBlockInfo
{
    ...
    //新增，在TransportBlockInfo中添加RB级信道增益记录
    std::map<uint32_t, std::complex<double>> rbChannelGain; // 新增：RB索引 -> 信道增益
};
...
struct RxPacketTraceParams
{
    RxPacketTraceParams(TransportBlockInfo tbInfo, 
        ...
        : m_cellId(cellId),
          ...
          m_rbBitmap(tbInfo.m_expected.m_rbBitmap), // 新增此行
          m_cqi(cqi){};
    uint64_t m_cellId{0};
    uint16_t m_rnti{0};
