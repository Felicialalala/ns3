...

{
    ...
    rxParams->spectrumChannelMatrix = GenSpectrumChannelMatrix(rxParams->psd,
                                                               ...
                                                               isReverse);
    // 存储所有rb的信道矩阵
    rxParams->allRbSpectrumChannelMatrix = GenSpectrumChannelMatrixWithoutPSD(rxParams->psd,
                                                            longTerm,
                                                            channelMatrix,
                                                            channelParams,
                                                            doppler,
                                                            ...
                                                            numRxPorts,
                                                            isReverse);
    // if (rxParams->allRbSpectrumChannelMatrix)
    // {
    //     static std::ofstream rbOutfile;
    //     const std::string rbFilename = "/home/yangyiting/workspace/ns-3-allinone/ns-3.42/output/rb_channel_matrix.csv";
        
    //     if (!rbOutfile.is_open()) {
    //         rbOutfile.open(rbFilename, std::ios::out | std::ios::app);
    //         rbOutfile << "rnti,rb,tx,rx,real,imag\n";
    //     }

    //     const size_t numRbs = rxParams->allRbSpectrumChannelMatrix->GetNumPages();
    //     const size_t numTx = rxParams->allRbSpectrumChannelMatrix->GetNumCols();
    //     const size_t numRx = rxParams->allRbSpectrumChannelMatrix->GetNumRows();

    //     for (size_t rb = 0; rb < numRbs; ++rb) {
    //         for (size_t tx = 0; tx < numTx; ++tx) {
    //             for (size_t rx = 0; rx < numRx; ++rx) {
    //                 const auto& h = rxParams->allRbSpectrumChannelMatrix->operator()(rx, tx, rb);
    ...
    bool isReverse) const
{
    ...
    size_t iRb = 0;
    // Compute the frequency-domain channel matrix 频域信道矩阵计算
...
// 自定义函数，目的是生成全部rb上的信道矩阵
// 不考虑inPsd上sqrtVit = sqrt(*vit)的变量相乘
// 修改后相当于假设所有子载波上的输入信号能量为1，可能影响后续的信噪比计算和接收机处理
// 适合需要分析原始信道响应（不考虑发射端功率分配）的场景
Ptr<MatrixBasedChannelModel::Complex3DVector>
ThreeGppSpectrumPropagationLossModel::GenSpectrumChannelMatrixWithoutPSD(
    Ptr<SpectrumValue> inPsd,//功率谱密度对象，描述信号在不同子载波上的能量分布
    Ptr<const MatrixBasedChannelModel::Complex3DVector> longTerm,
    Ptr<const MatrixBasedChannelModel::ChannelMatrix> channelMatrix,
    Ptr<const MatrixBasedChannelModel::ChannelParams> channelParams,
    ...
    while (vit != inPsd->ValuesEnd())
    {
        ...
        for (auto rxPortIdx = 0; rxPortIdx < numRxPorts; rxPortIdx++)
        {
            for (auto txPortIdx = 0; txPortIdx < numTxPorts; txPortIdx++)
            {
                ...
                // 移除sqrtVit乘法，直接使用子带增益
                chanSpct->Elem(rxPortIdx, txPortIdx, iRb) = subsbandGain; 
            }
        }
        vit++;
        iRb++;
